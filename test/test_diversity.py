"""Pytest golden suite for viz.diversity_derived."""
...
@pytest.mark.parametrize('geoid', GEOIDS)
def test_diversity_bounds(conn, geoid):
  row = fetch_row(conn, geoid)
  point = row['race_diversity_index']
  lo = row['race_diversity_index_lo90']
  hi = row['race_diversity_index_hi90']

  if point is None:
    assert lo is None and hi is None, f'[{geoid}] expected NULL CI when point is NULL'
    return

  assert 0.0 <= float(point) <= 1.0, f'[{geoid}] point out of range: {point}'
  assert lo is not None and hi is not None, f'[{geoid}] expected non-NULL CI for computed row'
  assert 0.0 <= float(lo) <= 1.0, f'[{geoid}] lo out of range: {lo}'
  assert 0.0 <= float(hi) <= 1.0, f'[{geoid}] hi out of range: {hi}'
  assert float(lo) <= float(point) <= float(hi), (
    f'[{geoid}] expected lo <= point <= hi, got lo={lo}, point={point}, hi={hi}'
  )
